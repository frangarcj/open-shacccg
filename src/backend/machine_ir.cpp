...
MachineOperand MachineProgram::physical(usse::RegisterRef reg, MachineType type) const {
    return MachineOperand::physical_value(reg.bank, reg.num, type);
}
...
bool compile_machine_program(const MachineProgram &program, MachineCompileResult &out) {
    ...
    for (uint32_t instruction_index = 0; instruction_index < program.instructions().size(); ++instruction_index) {
        ...
        case MachineOpcode::Vector: {
            ...
            if (swizzle != MachineVectorSwizzle::Identity && swizzle != MachineVectorSwizzle::PositionXY11) {
